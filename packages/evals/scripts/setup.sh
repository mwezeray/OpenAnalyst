...
build_extension() {
  echo "🔨 Building the OpenAnalyst extension..."
  pnpm -w vsix -- --out ../bin/oa-code-$(git rev-parse --short HEAD).vsix || exit 1
  code --install-extension ../../bin/oa-code-$(git rev-parse --short HEAD).vsix || exit 1
  cd evals
}
...
if ! code --list-extensions 2>/dev/null | grep -q "oacode.Oa-Code"; then
  code --install-extension oacode.Oa-Code &>/dev/null || exit 1
fi
...
echo -n "🗄️ Syncing OpenAnalyst evals database... "
...
current_version=$(code --list-extensions --show-versions 2>/dev/null | grep oacode)
read -p "💻 Do you want to build a new version of the OpenAnalyst extension? [currently $current_version] (y/N): " build_extension
