...
export abstract class BaseTelemetryClient implements TelemetryClient {
	...
	protected async getEventProperties(event: TelemetryEvent): Promise<TelemetryEvent["properties"]> {
		...
		if (provider) {
			try {
				...
				providerProperties.exception = error instanceof Error ? error.stack || error.message : String(error) // oacode_change
			}
		}
	...
	public updateIdentity(_oacodeToken: string): Promise<void> {
