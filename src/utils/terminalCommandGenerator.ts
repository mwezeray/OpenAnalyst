...
export async function generateTerminalCommand(options: TerminalCommandGeneratorOptions): Promise<void> {
	...
	try {
		...
		if (!activeTerminal) {
			vscode.window.showErrorMessage(t("oacode:terminalCommandGenerator.noActiveTerminal"))
			return
		}
...
async function showWarningIfNeeded(context: vscode.ExtensionContext): Promise<boolean> {
	...
	if (!warningAcknowledged) {
		const warningChoice = await vscode.window.showInformationMessage(
			t("oacode:terminalCommandGenerator.warningDialog.title"),
			{ modal: true, detail: t("oacode:terminalCommandGenerator.warningDialog.message") },
			t("oacode:terminalCommandGenerator.warningDialog.okButton"),
			t("oacode:terminalCommandGenerator.warningDialog.cancelButton"),
		)

		if (warningChoice !== t("oacode:terminalCommandGenerator.warningDialog.okButton")) {
			return false
		}
...
async function getUserInput(): Promise<string | undefined> {
	return await vscode.window.showInputBox({
		prompt: t("oacode:terminalCommandGenerator.inputPrompt"),
		placeHolder: t("oacode:terminalCommandGenerator.inputPlaceholder"),
		ignoreFocusOut: true,
	})
}
	...
	await vscode.window.withProgress(
		{
			...
			title: t("oacode:terminalCommandGenerator.generatingProgress"),
			cancellable: false,
		},
		async () => {
			...
			vscode.window.showInformationMessage(
				t("oacode:terminalCommandGenerator.commandGenerated", { command: cleanCommand }),
			)
		},
	)
}
...
function handleError(error: unknown, outputChannel: vscode.OutputChannel, context: string): void {
	...
	if (context === "generateTerminalCommand") {
		vscode.window.showErrorMessage(t("oacode:terminalCommandGenerator.generationFailed", { error: errorMessage }))
	} else {
