...
/**
 * Checks for a file in .oacode/launchPrompt.md and runs it immediately if it exists.
 */
export async function checkAndRunAutoLaunchingTask(context: vscode.ExtensionContext): Promise<void> {
	...
	const promptFilePath = vscode.Uri.joinPath(workspaceFolderUri, ".oacode", "launchPrompt.md")
	...
	try {
		...
		vscode.commands.executeCommand("oa-code.newTask", { prompt })
	} catch (error) {
		if (error instanceof vscode.FileSystemError && error.code === "FileNotFound") {
			return // File not found, which is expected if no launchPrompt.md exists
		}
		console.error(`Error running .oacode/launchPrompt.md: ${error}`)
	}
}
