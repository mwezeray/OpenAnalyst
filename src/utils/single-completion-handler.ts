...
import { buildApiHandler, SingleCompletionHandler, ApiHandler } from "../api" //oacode_change
...
export async function singleCompletionHandler(apiConfiguration: ProviderSettings, promptText: string): Promise<string> {
	...
	if (!("completePrompt" in handler)) {
		// oacode_change start - stream responses for handlers without completePrompt
...
// oacode_change start - Stream responses using createMessage
...
// oacode_change end - streamResponseFromHandler
