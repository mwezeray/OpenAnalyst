...
const LMSTUDIO_TIMEOUT_MS = 3_600_000 // oacode_change
...
export class LmStudioHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		this.client = new OpenAI({
			...
			apiKey: "noop",
			timeout: LMSTUDIO_TIMEOUT_MS, // oacode_change
			fetch: fetchWithTimeout(LMSTUDIO_TIMEOUT_MS), // oacode_change
		})
	}
	...
	override async *createMessage(
		...
		try {
			...
			throw new Error(
				"Please check the LM Studio developer logs to debug what went wrong. You may need to load the model with a larger context length to work with OpenAnalyst's prompts.",
			)
		}
	}
