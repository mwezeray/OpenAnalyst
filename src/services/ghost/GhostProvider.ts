...
export class GhostProvider {
	...
	public async promptCodeSuggestion() {
		...
		const userInput = await vscode.window.showInputBox({
			prompt: t("oacode:ghost.input.title"),
			placeHolder: t("oacode:ghost.input.placeholder"),
		})
		if (!userInput) {
			return
		}
	...
	private async updateGlobalContext() {
		...
		await vscode.commands.executeCommand("setContext", "oacode.ghost.hasSuggestions", hasSuggestions)
		await vscode.commands.executeCommand("setContext", "oacode.ghost.isProcessing", this.isProcessing)
		await vscode.commands.executeCommand(
			"setContext",
			"oacode.ghost.enableQuickInlineTaskKeybinding",
			this.settings?.enableQuickInlineTaskKeybinding || false,
		)
		await vscode.commands.executeCommand(
			"setContext",
			"oacode.ghost.enableSmartInlineTaskKeybinding",
	...
	public async showIncompatibilityExtensionPopup() {
		const message = t("oacode:ghost.incompatibilityExtensionPopup.message")
		const disableCopilot = t("oacode:ghost.incompatibilityExtensionPopup.disableCopilot")
		const disableInlineAssist = t("oacode:ghost.incompatibilityExtensionPopup.disableInlineAssist")
		...
		if (response === disableCopilot) {
			...
			await vscode.commands.executeCommand<any>("oa-code.ghost.disable")
		}
	}
