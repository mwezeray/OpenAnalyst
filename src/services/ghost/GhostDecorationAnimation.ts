...
export class GhostDecorationAnimation {
	...
	private readonly animationFrames = ["█", "K█", "KI█", "KIL█", "OA█"]
	...
	private updateDecorationText(): void {
		...
		if (this.animationState === this.animationFrames.length - 1) {
			// Show either the full frame with block, or just "OA" without block
			text = this.isBlockVisible ? this.animationFrames[this.animationState] : "OA"
		} else {
