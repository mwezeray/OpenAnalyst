...
export class CommitMessageProvider {
	...
	public async activate(): Promise<void> {
		this.outputChannel.appendLine(t("oacode:commitMessage.activated"))

		try {
			await this.providerSettingsManager.initialize()
		} catch (error) {
			this.outputChannel.appendLine(t("oacode:commitMessage.gitInitError", { error }))
		}

		// Register the command
		const disposable = vscode.commands.registerCommand(
			"oa-code.generateCommitMessage",
		...
		await vscode.window.withProgress(
			{
				...
				title: t("oacode:commitMessage.generating"),
				cancellable: false,
			},
			async (progress) => {
				try {
					...
					if (changes.length === 0) {
						...
						if (changes.length > 0) {
							vscode.window.showInformationMessage(t("oacode:commitMessage.generatingFromUnstaged"))
						} else {
							vscode.window.showInformationMessage(t("oacode:commitMessage.noChanges"))
							return
						}
					}

					// Report initial progress after gathering changes (10% of total)
					progress.report({ increment: 10, message: t("oacode:commitMessage.generating") })
					...
					const onDiffProgress = (percentage: number) => {
						...
						if (increment > 0) {
							progress.report({ increment, message: t("oacode:commitMessage.generating") })
					...
					vscode.window.showErrorMessage(t("oacode:commitMessage.generationFailed", { errorMessage }))
	...
	private async callAIForCommitMessageWithProgress(
		...
		const progressInterval = setInterval(() => {
			...
			progress.report({ increment: increment, message: t("oacode:commitMessage.generating") })
