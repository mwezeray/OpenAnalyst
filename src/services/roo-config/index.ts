...
import fsSync from "fs" // oacode_change
...
export function getGlobalRooDirectory(): string {
	...
	return path.join(homeDir, ".oacode") // oacode_change
}
...
export function getProjectRooDirectoryForCwd(cwd: string): string {
	// oacode_change start
	const oaDir = path.join(cwd, ".oacode")
	const rooDir = path.join(cwd, ".roo")
	if (fsSync.existsSync(rooDir) && !fsSync.existsSync(oaDir)) {
		return rooDir
	}
	return oaDir
	// oacode_change end
