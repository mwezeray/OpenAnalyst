...
import { NotificationService } from "./oacode/NotificationService"
...
export class McpHub {
	...
	readonly oaNotificationService = new NotificationService()
	...
	private async watchProjectMcpFile(): Promise<void> {
		...
		const projectMcpPattern = new vscode.RelativePattern(workspaceFolder, ".oacode/mcp.json")
  ...
  "mcpServers": {
	...
	private async getProjectMcpPath(): Promise<string | null> {
		...
		const workspaceFolder = vscode.workspace.workspaceFolders[0]
		// oacode_change
		// First, we try the standard location: .oacode/mcp.json
		// If not found, fall back to .mcp.json in the project root
		const projectMcpDir = path.join(workspaceFolder.uri.fsPath, ".oacode")
		...
		try {
			...
			// If not found in .oacode/, fall back to .mcp.json in root directory
	...
	private async connectToServer(
		...
		try {
			...
			this.oaNotificationService.connect(name, connection.client)
