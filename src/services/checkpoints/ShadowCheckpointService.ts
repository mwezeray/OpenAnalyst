...
export abstract class ShadowCheckpointService extends EventEmitter {
	...
	public async initShadowGit(onInit?: () => Promise<void>) {
		...
		if (await fileExistsAtPath(this.dotGitDir)) {
			...
			await git.addConfig("user.name", "OpenAnalyst")
	...
	private async stageAll(git: SimpleGit) {
		try {
			...
			reportError(`${this.constructor.name}#stageAll`, error) // oacode_change
		}
	}
	...
	private async hasNestedGitRepositories(): Promise<boolean> {
		try {
			...
			reportError(`${this.constructor.name}#hasNestedGitRepositories`, error) // oacode_change
	...
	private async getShadowGitConfigWorktree(git: SimpleGit) {
		if (!this.shadowGitConfigWorktree) {
			try {
				...
				reportError(`${this.constructor.name}#getShadowGitConfigWorktree`, error) // oacode_change
			}
		}
	...
	public async getDiff({ from, to }: { from?: string; to?: string }): Promise<CheckpointDiff[]> {
		...
		for (const file of files) {
			...
			const before = await this.git.show([`${from}:${relPath}`]).catch((err) => {
				reportError(`[${this.constructor.name}#getDiff:git.show:before`, err) // oacode_change
				return ""
			})

			const after = to
				? await this.git.show([`${to}:${relPath}`]).catch((err) => {
						reportError(`[${this.constructor.name}#getDiff:git.show:after`, err) // oacode_change
						return ""
					})
				: await fs.readFile(absPath, "utf8").catch((err) => {
						reportError(`[${this.constructor.name}#getDiff:readFile`, err) // oacode_change
						return ""
					})
	...
	public static async deleteBranch(git: SimpleGit, branchName: string) {
		...
		if (currentBranch === branchName) {
			...
			try {
				...
				reportError(`${this.constructor.name}#deleteBranch`, error) // oacode_change
