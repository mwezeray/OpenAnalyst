...
export async function reportBugTool(
	...
	try {
		if (block.partial) {
			...
			const oacodeVersion = vscode.extensions.getExtension("oacode.oa-code")?.packageJSON.version || "Unknown"
			...
			if (text || images?.length) {
				...
				try {
					...
					await createAndOpenGitHubIssue("Oa-Org", "oacode", "bug_report.yml", params)
				} catch (error) {
