...
import { t } from "../../i18n"
import { McpToolCallResponse } from "../../shared/mcp" // oacode_change
import { summarizeSuccessfulMcpOutputWhenTooLong } from "./oacode" // oacode_change
...
// oacode_change: make async, add task parameter
async function processToolContent(task: Task, toolResult: McpToolCallResponse): Promise<string> {
	if (!toolResult?.content || toolResult.content.length === 0) {
		return ""
	}

	const outputText = toolResult.content // oacode_change: introduce const
		.map((item: any) => {
			if (item.type === "text") {
				return item.text
			}
...
async function executeToolAndProcessResult(
	...
	if (toolResult) {
		// oacode_change: await, add api parameter
