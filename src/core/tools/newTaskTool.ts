...
export async function newTaskTool(
	...
	try {
		if (block.partial) {
			...
			// oacode_change start: Switch to the desired mode BEFORE creating the task
			...
			if (!newCline) {
				await provider.handleModeSwitch(cline.pausedModeSlug) // oacode_change: if task creation failed, switch back to the parent's mode
				pushToolResult(t("tools:newTask.errors.policy_restriction"))
				return
			}

			// oacode_change start: Switch to the desired mode BEFORE creating the task above
