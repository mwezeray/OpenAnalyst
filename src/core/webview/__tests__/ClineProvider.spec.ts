...
vi.mock("vscode", () => ({
	...
	env: {
		uriScheme: "vscode",
		language: "en",
		uiKind: 1, // oacode_change Desktop
	...
	test("postMessageToWebview sends message to webview", async () => {
		...
		const mockState: ExtensionState = {
			...
			apiConfiguration: {
				// oacode_change start
				apiProvider: "oacode",
				oacodeModel: oacodeDefaultModelId,
				oacodeToken: "oacode-token",
				// oacode_change end
			},
...
describe("Project MCP Settings", () => {
	...
	test.skip("handles openProjectMcpSettings message", async () => {
		...
		expect(mockedFs.mkdir).toHaveBeenCalledWith("/test/workspace/.oacode", { recursive: true })

		// Verify file was created with default content
		expect(safeWriteJson).toHaveBeenCalledWith("/test/workspace/.roo/mcp.json", { mcpServers: {} })

		// Check that openFile was called
		expect(openFileSpy).toHaveBeenCalledWith("/test/workspace/.oacode/mcp.json")
	})
	...
	test.skip("handles openProjectMcpSettings file creation error", async () => {
		...
		expect(vscode.window.showErrorMessage).toHaveBeenCalledWith(
			// oacode_change
			expect.stringContaining("Failed to create or open .oacode/mcp.json"),
		)
	})
})
...
describe("ClineProvider - Router Models", () => {
	...
	test("handles requestRouterModels with successful responses", async () => {
		...
		expect(getModels).toHaveBeenCalledWith({ provider: "openrouter", apiKey: "openrouter-key" }) // oacode_change: apiKey
		...
		expect(mockPostMessage).toHaveBeenCalledWith({
			...
			routerModels: {
				...
				unbound: mockModels,
				litellm: mockModels,
				"oacode-openrouter": mockModels,
				ollama: mockModels, // oacode_change
				lmstudio: {},
			},
		})
	})
	...
	test("handles requestRouterModels with individual provider failures", async () => {
		...
		vi.mocked(getModels)
			...
			.mockRejectedValueOnce(new Error("Oacode-OpenRouter API error")) // oacode-openrouter fail
			.mockRejectedValueOnce(new Error("Ollama API error")) // oacode_change
		...
		expect(mockPostMessage).toHaveBeenCalledWith({
			type: "singleRouterModelFetchResponse",
			success: false,
			error: "Oacode-OpenRouter API error",
			values: { provider: "oacode-openrouter" },
		})
