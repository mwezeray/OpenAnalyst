...
import { allowedExtensions } from "../../shared/oacode/rules"
...
function getRuleDirectoryPath(baseDir: string, ruleType: "rule" | "workflow") {
	return ruleType === "workflow"
		...
		: path.join(baseDir, GlobalFileNames.oaRules)
}
...
export async function createRuleFile(
	...
	if (!workspacePath && !isGlobal) {
		vscode.window.showErrorMessage(t("oacode:rules.errors.noWorkspaceFound"))
		return
	}
	...
	if (await fileExistsAtPath(filePath)) {
		vscode.window.showErrorMessage(t("oacode:rules.errors.fileAlreadyExists", { filename }))
		return
	}
...
${t("oacode:rules.templates.workflow.description")}

${t("oacode:rules.templates.workflow.stepsHeader")}

1. ${t("oacode:rules.templates.workflow.step1")}
2. ${t("oacode:rules.templates.workflow.step2")}
`
}
...
${t("oacode:rules.templates.rule.description")}

${t("oacode:rules.templates.rule.guidelinesHeader")}

- ${t("oacode:rules.templates.rule.guideline1")}
- ${t("oacode:rules.templates.rule.guideline2")}
`
}

export async function deleteRuleFile(rulePath: string): Promise<void> {
	const deleteAction = t("oacode:rules.actions.delete")
	const filename = path.basename(rulePath)
	const result = await vscode.window.showWarningMessage(
		t("oacode:rules.actions.confirmDelete", { filename }),
		{ modal: true },
		deleteAction,
	)
	...
	if (result === deleteAction) {
		...
		vscode.window.showInformationMessage(t("oacode:rules.actions.deleted", { filename }))
	}
}
