...
describe("custom-instructions global .roo support", () => {
	...
	const globalRooDir = path.join(mockHomeDir, ".oacode")
	const projectRooDir = path.join(mockCwd, ".oacode")
	...
	describe("loadRuleFiles", () => {
		...
		it("should fall back to legacy .oacoderules file when no .roo/rules directories exist", async () => {
			...
			expect(result).toContain("# Rules from .oacoderules:")
		...
		it("should return empty string when no rules exist anywhere", async () => {
			...
			mockReadFile
				.mockResolvedValueOnce("") // .oacoderules returns empty (simulating ENOENT caught by safeReadFile)
	...
	describe("addCustomInstructions mode-specific rules", () => {
		it("should load global and project mode-specific rules", async () => {
			...
			mockReadFile
				...
				.mockResolvedValueOnce("") // .oacoderules legacy file (empty)
		...
		it("should fall back to legacy mode-specific files when no mode directories exist", async () => {
			...
			mockReadFile
				...
				.mockResolvedValueOnce("") // AGENTS.md file (empty)
				.mockResolvedValueOnce("") // generic .oacoderules (empty)
				.mockResolvedValueOnce("") // generic .roorules (empty)
				.mockResolvedValueOnce("") // generic .clinerules (empty)

			const result = await addCustomInstructions("", "", mockCwd, mode)

			expect(result).toContain("# Rules from .oacoderules-code:")
