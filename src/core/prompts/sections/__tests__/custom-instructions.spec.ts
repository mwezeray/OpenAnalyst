...
describe("loadRuleFiles", () => {
	...
	it("should not combine content from multiple rule files when they exist", async () => {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().endsWith(".oacoderules")) {
	...
	it("should use .oacode/rules/ directory when it exists and has files", async () => {
		// Simulate .oacode/rules directory exists
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				parentPath: "/fake/path/.oacode/rules",
			},
			{
				name: "file2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			if (
				normalizedPath.includes("/fake/path/.oacode/rules/file1.txt") ||
				normalizedPath.includes("/fake/path/.oacode/rules/file2.txt")
			) {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.oacode/rules/file1.txt") {
				return Promise.resolve("content of file1")
			}
			if (normalizedPath === "/fake/path/.oacode/rules/file2.txt") {
		...
		const expectedPath1 =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\rules\\file1.txt"
				: "/fake/path/.oacode/rules/file1.txt"
		const expectedPath2 =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\rules\\file2.txt"
				: "/fake/path/.oacode/rules/file2.txt"
		...
		const expectedRulesDir =
			process.platform === "win32" ? "\\fake\\path\\.oacode\\rules" : "/fake/path/.oacode/rules"
	...
	it("should fall back to .oacoderules when .oacode/rules/ is empty", async () => {
		// Simulate .oacode/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
	...
	it("should read files from nested subdirectories in .oacode/rules/", async () => {
		// Simulate .oacode/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			...
			},
			{
				...
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.oacode/rules/subdir",
			},
			{
				name: "nested2.txt",
				isFile: () => true,
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.oacode/rules/subdir/subdir2",
			},
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.oacode/rules/root.txt") {
				return Promise.resolve("root file content")
			}
			if (normalizedPath === "/fake/path/.oacode/rules/subdir/nested1.txt") {
				return Promise.resolve("nested file 1 content")
			}
			if (normalizedPath === "/fake/path/.oacode/rules/subdir/subdir2/nested2.txt") {
		...
		const expectedRootPath =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\rules\\root.txt"
				: "/fake/path/.oacode/rules/root.txt"
		const expectedNested1Path =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\rules\\subdir\\nested1.txt"
				: "/fake/path/.oacode/rules/subdir/nested1.txt"
		const expectedNested2Path =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\rules\\subdir\\subdir2\\nested2.txt"
				: "/fake/path/.oacode/rules/subdir/subdir2/nested2.txt"
...
describe("addCustomInstructions", () => {
	...
	it("should combine all instruction types when provided", async () => {
		// Simulate no .oacode/rules-test-mode directory
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
		...
		expect(result).toContain("Rules from .oacoderules-test-mode:\nmode specific rules")
	})
	...
	it("should use .oacode/rules-test-mode/ directory when it exists and has files", async () => {
		// Simulate .oacode/rules-test-mode directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				isFile: () => true,
				isSymbolicLink: () => false,
				parentPath: "/fake/path/.oacode/rules-test-mode",
			},
			{
				name: "rule2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			const normalizedPath = path.toString().replace(/\\/g, "/")
			if (
				normalizedPath.includes("/fake/path/.oacode/rules-test-mode/rule1.txt") ||
				normalizedPath.includes("/fake/path/.oacode/rules-test-mode/rule2.txt")
			) {
				return Promise.resolve({
					isFile: vi.fn().mockReturnValue(true),
				}) as any
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.oacode/rules-test-mode/rule1.txt") {
				return Promise.resolve("mode specific rule 1")
			}
			if (normalizedPath === "/fake/path/.oacode/rules-test-mode/rule2.txt") {
		...
		const expectedTestModeDir =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\rules-test-mode"
				: "/fake/path/.oacode/rules-test-mode"
		const expectedRule1Path =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\rules-test-mode\\rule1.txt"
				: "/fake/path/.oacode/rules-test-mode/rule1.txt"
		const expectedRule2Path =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\rules-test-mode\\rule2.txt"
				: "/fake/path/.oacode/rules-test-mode/rule2.txt"
	...
	it("should fall back to .oacoderules-test-mode when .oacode/rules-test-mode/ does not exist", async () => {
		// Simulate .oacode/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })

		// Simulate .oacoderules-test-mode exists
		readFileMock.mockImplementation((filePath: PathLike) => {
			if (filePath.toString().includes(".oacoderules-test-mode")) {
	...
	it("should correctly format content from directories when using .oacode/rules-test-mode/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "rule1.txt", isFile: () => true, parentPath: "/fake/path/.oacode/rules-test-mode" },
		] as any)
		readFileMock.mockReset()
...
describe("Rules directory reading", () => {
	it.skipIf(process.platform === "win32")("should follow symbolic links in the rules directory", async () => {
			...
			.mockResolvedValueOnce([
				{
					...
					isFile: () => true,
					parentPath: "/fake/path/.oacode/rules/symlink-target-dir",
				},
			] as any)
		...
		statMock.mockImplementation((path: string) => {
			// For directory check
			if (path === "/fake/path/.oacode/rules" || path.endsWith("dir")) {
				return Promise.resolve({
					isDirectory: vi.fn().mockReturnValue(true),
					isFile: vi.fn().mockReturnValue(false),
				} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.oacode/rules/regular.txt") {
				return Promise.resolve("regular file content")
			}
			if (normalizedPath === "/fake/path/.oacode/symlink-target.txt") {
				return Promise.resolve("symlink target content")
			}
			if (normalizedPath === "/fake/path/.oacode/rules/symlink-target-dir/subdir_link.txt") {
				return Promise.resolve("regular file content under symlink target dir")
			}
			if (normalizedPath === "/fake/path/.oacode/nested-symlink-target.txt") {
		...
		const expectedRegularPath =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\rules\\regular.txt"
				: "/fake/path/.oacode/rules/regular.txt"
		const expectedSymlinkPath =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\symlink-target.txt"
				: "/fake/path/.oacode/symlink-target.txt"
		const expectedSubdirPath =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\rules\\symlink-target-dir\\subdir_link.txt"
				: "/fake/path/.oacode/rules/symlink-target-dir/subdir_link.txt"
		const expectedNestedPath =
			process.platform === "win32"
				? "\\fake\\path\\.oacode\\nested-symlink-target.txt"
				: "/fake/path/.oacode/nested-symlink-target.txt"
		...
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.oacode/rules/link.txt")
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.oacode/rules/link_dir")

		// Verify both files were read
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.oacode/rules/regular.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.oacode/symlink-target.txt", "utf-8")
		expect(readFileMock).toHaveBeenCalledWith(
			"/fake/path/.oacode/rules/symlink-target-dir/subdir_link.txt",
			"utf-8",
		)
		expect(readFileMock).toHaveBeenCalledWith("/fake/path/.oacode/nested-symlink-target.txt", "utf-8")
	})
	beforeEach(() => {
		vi.clearAllMocks()
	})
	...
	it.skipIf(process.platform === "win32")("should correctly format multiple files from directory", async () => {
		...
		// Simulate listing files
		readdirMock.mockResolvedValueOnce([
			{ name: "file1.txt", isFile: () => true, parentPath: "/fake/path/.oacode/rules" },
			{ name: "file2.txt", isFile: () => true, parentPath: "/fake/path/.oacode/rules" },
			{ name: "file3.txt", isFile: () => true, parentPath: "/fake/path/.oacode/rules" },
		] as any)
		...
		statMock.mockImplementation((path) => {
			...
			expect([
				"/fake/path/.oacode/rules/file1.txt",
				"/fake/path/.oacode/rules/file2.txt",
				"/fake/path/.oacode/rules/file3.txt",
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.oacode/rules/file3.txt") {
