...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	// oacode_change
	return path.join(cwd, ".oacode", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .oacode/system-prompt-[mode slug]
...
/**
 * Ensures the .oacode directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	// oacode_change
	const rooDir = path.join(cwd, ".oacode")
