...
import { hasAnyToggles, loadEnabledRules } from "./oa"
...
let hasShownNonOacodeRulesMessage = false
// oacode_change end
...
import { ClineRulesToggles } from "../../../shared/cline-rules" // oacode_change
...
async function readTextFilesFromDirectory(dirPath: string): Promise<Array<{ filename: string; content: string }>> {
	try {
		...
		// oacode_change, must be imported at submodule level because the module is imported in the webview-ui
...
export async function loadRuleFiles(cwd: string): Promise<string> {
	...
	const ruleFiles = [".oacoderules", ".roorules", ".clinerules"]

	for (const file of ruleFiles) {
		const content = await safeReadFile(path.join(cwd, file))
		if (content) {
			if (file !== ".oacoderules" && vscodeAPI && !hasShownNonOacodeRulesMessage) {
				// oacode_change: show message to move to .oacode/rules/
				vscodeAPI.window.showWarningMessage(
					`Loading non-Oacode rules from ${file}, consider moving to .oacode/rules/`,
				)
				hasShownNonOacodeRulesMessage = true
			} // oacode_change end
			return `\n# Rules from ${file}:\n${content}\n`
		}
	}
...
export async function addCustomInstructions(
	...
	cwd: string,
	mode: string,
	// oacode_change begin: rule toggles
	options: {
		language?: string
	...
	if (mode) {
		...
		if (modeRules.length > 0) {
			...
			const rooModeRuleFile = `.oacoderules-${mode}`
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".oacode", `rules-${mode}`))) {
	...
	// oacode_change start: rule toggles
