...
describe("RooIgnoreController", () => {
	...
	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .oacodeignore exists
		 */
		it("should load .oacodeignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .oacodeignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".oacodeignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".oacodeignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .oacodeignore doesn't exist
		 */
		it("should allow all access when .oacodeignore doesn't exist", async () => {
			// Setup mocks to simulate missing .oacodeignore file
		 ...
		 */
		it("should set up file watcher for .oacodeignore changes", async () => {
			// Check that watcher was created with correct pattern
			expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
				expect.objectContaining({
					base: TEST_CWD,
					pattern: ".oacodeignore",
				}),
			)
		 ...
		 */
		it("should handle errors when loading .oacodeignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .oacodeignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .oacodeignore exists
		 */
		it("should allow all access when no .oacodeignore content", async () => {
			// Create a new controller with no .oacodeignore
			mockFileExists.mockResolvedValue(false)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .oacodeignore exists
		 */
		it("should allow all commands when no .oacodeignore exists", async () => {
			// Create a new controller with no .oacodeignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RooIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .oacodeignore
		 */
		it("should generate formatted instructions when .oacodeignore exists", async () => {
			// Setup .oacodeignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
			...
			expect(instructions).toContain("# .oacodeignore")
		 ...
		 * Tests behavior when no .oacodeignore exists
		 */
		it("should return undefined when no .oacodeignore exists", async () => {
			// Setup no .oacodeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .oacodeignore is created
		 */
		it("should reload .oacodeignore when file is created", async () => {
			// Setup initial state without .oacodeignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .oacodeignore
			controller = new RooIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .oacodeignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .oacodeignore is changed
		 */
		it("should reload .oacodeignore when file is changed", async () => {
			// Setup initial state with .oacodeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .oacodeignore is deleted
		 */
		it("should reset when .oacodeignore is deleted", async () => {
			// Setup initial state with .oacodeignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
