...
const resendMessageSequence = async (
	...
	if (!currentCline || currentCline.taskId !== taskId) {
		...
		vscode.window.showErrorMessage(t("oacode:userFeedback.message_update_failed"))
		return false
	}
...
export const fetchOacodeNotificationsHandler = async (provider: ClineProvider) => {
	try {
		const { apiConfiguration } = await provider.getState()
		const oacodeToken = apiConfiguration?.oacodeToken

		if (!oacodeToken || apiConfiguration?.apiProvider !== "oacode") {
			provider.postMessageToWebview({
				type: "oacodeNotificationsResponse",
				notifications: [],
			})
			return
		}

		const response = await axios.get("https://oacode.ai/api/users/notifications", {
			headers: {
		...
		provider.log(`Error fetching Oacode notifications: ${error.message}`)
		provider.postMessageToWebview({
			type: "oacodeNotificationsResponse",
			notifications: [],
		})
	...
	try {
		...
		if (revert) {
			...
			if (checkpointMessage && checkpointMessage.text) {
				...
				vscode.window.showErrorMessage(t("oacode:userFeedback.no_checkpoint_found"))
			}
		}
		...
		if (success) {
			vscode.window.showInformationMessage(t("oacode:userFeedback.message_updated"))
		}
	} catch (error) {
