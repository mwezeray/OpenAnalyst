...
type ClaudeCodeOptions = {
	systemPrompt: string
	systemPromptFile?: string // oacode_change
...
async function generateTempSystemPrompt(options: ClaudeCodeOptions): Promise<string | undefined> {
	...
	const tempFilePath = path.join(os.tmpdir(), `oacode-system-prompt-${uniqueId}.txt`)
...
export async function* runClaudeCode(
	...
	const systemPromptFile = await generateTempSystemPrompt(options) // oacode_change
	const claudePath = options.path || "claude"
	let process

	try {
		process = runProcess({ ...options, systemPromptFile }) // oacode_change
	} catch (error: any) {
...
function runProcess({
	...
	// const isWindows = os.platform() === "win32" oacode_change
	...
	const stdinData = JSON.stringify(messages) // oacode_change
