	...
	return (
		<>
			<h3 className="text-lg font-medium mb-0">
				<Trans i18nKey="oacode:virtualProvider.title">Virtual Quota Fallback Settings</Trans>
			</h3>
			<div className="text-sm text-vscode-descriptionForeground mb-4">
				<Trans i18nKey="oacode:virtualProvider.description">
			...
			<div className="space-y-1">
						...
						<div key={index} className="border border-vscode-settings-sashBorder rounded-md p-2">
							<div className="flex items-center justify-between mb-3">
								<label className="block font-medium">
									{index === 0
										? t("oacode:virtualProvider.primaryProfileLabel", { number: index + 1 })
										: t("oacode:virtualProvider.profileLabel", { number: index + 1 })}
								</label>
								<div className="flex items-center gap-1">
									...
									<VSCodeButton
										...
										disabled={index === 0}
										title={t("oacode:virtualProvider.moveProfileUp")}>
										<ChevronUp size={16} />
									</VSCodeButton>
									{/* Move Down Button */}
									<VSCodeButton
										...
										title={t("oacode:virtualProvider.moveProfileDown")}>
										<ChevronDown size={16} />
									</VSCodeButton>
									{/* Remove Button */}
									{profiles.length > 1 && (
										<VSCodeButton
											...
											title={t("oacode:virtualProvider.removeProfile")}>
											<TrashIcon />
										</VSCodeButton>
									)}
								</div>
							...
							<Select
								...
								<SelectTrigger className="w-full">
									<SelectValue placeholder={t("oacode:virtualProvider.selectProfilePlaceholder")} />
								</SelectTrigger>
								<SelectContent>
				...
				<div className="flex justify-center p-4">
					<VSCodeButton
						...
						<Trans i18nKey="oacode:virtualProvider.addProfile">Add Profile</Trans>
					</VSCodeButton>
				</div>
				...
				{availableProfiles.length === 0 ? (
					<div className="text-sm text-vscode-descriptionForeground text-center p-4 border border-vscode-settings-sashBorder rounded-md">
						<Trans i18nKey="oacode:virtualProvider.noProfilesAvailable">
			...
			<div className="p-4 border border-vscode-editorWarning-foreground rounded-md">
				<div className="text-md font-semibold text-vscode-editorWarning-foreground">
					<Trans i18nKey="oacode:virtualProvider.dangerZoneTitle">Danger Zone</Trans>
				</div>
				<p className="text-sm text-vscode-descriptionForeground mt-1 mb-3">
					<Trans i18nKey="oacode:virtualProvider.dangerZoneDescription">
						These actions are destructive and cannot be undone.
					</Trans>
				</p>
				<VSCodeButton appearance="secondary" onClick={() => onSetIsAlertOpen(true)}>
					<Trans i18nKey="oacode:virtualProvider.clearUsageData">Clear Usage Data</Trans>
				</VSCodeButton>
			</div>
			...
			<AlertDialog open={isAlertOpen} onOpenChange={onSetIsAlertOpen}>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>
							<Trans i18nKey="oacode:virtualProvider.confirmClearTitle">Are you sure?</Trans>
						</AlertDialogTitle>
						<AlertDialogDescription>
							<Trans i18nKey="oacode:virtualProvider.confirmClearDescription">
		...
		<div className="space-y-4 p-2 rounded-md mt-2">
			<div>
				<label className="block text-sm font-medium mb-2">
					<Trans i18nKey="oacode:virtualProvider.tokensLabel">Tokens</Trans>
				</label>
				<div className="grid grid-cols-3 gap-x-4">
					<div>
						<label className="block text-xs text-vscode-descriptionForeground mb-1">
							<Trans i18nKey="oacode:virtualProvider.perMinute">Per Minute</Trans>
						</label>
						<VSCodeTextField
					...
					<div>
						<label className="block text-xs text-vscode-descriptionForeground mb-1">
							<Trans i18nKey="oacode:virtualProvider.perHour">Per Hour</Trans>
						</label>
						<VSCodeTextField
					...
					<div>
						<label className="block text-xs text-vscode-descriptionForeground mb-1">
							<Trans i18nKey="oacode:virtualProvider.perDay">Per Day</Trans>
						</label>
						<VSCodeTextField
			...
			<div>
				<label className="block text-sm font-medium mb-2">
					<Trans i18nKey="oacode:virtualProvider.requestsLabel">Requests</Trans>
				</label>
				<div className="grid grid-cols-3 gap-x-4">
					<div>
						<label className="block text-xs text-vscode-descriptionForeground mb-1">
