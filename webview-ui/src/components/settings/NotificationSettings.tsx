import { HTMLAttributes, useMemo } from "react" // oacode_change
...
type NotificationSettingsProps = HTMLAttributes<HTMLDivElement> & {
	...
	areSettingsCommitted?: boolean // oacode_change
...
export const NotificationSettings = ({
	...
	areSettingsCommitted, // oacode_change
	setCachedStateField,
	...props
	...
	const onTestNotificationClick = () => {
		vscode.postMessage({
			...
			notificationOptions: {
				title: t("oacode:settings.systemNotifications.testTitle"),
				message: t("oacode:settings.systemNotifications.testMessage"),
			},
		})
	}
			...
			<Section>
				...
				<div>
					<VSCodeCheckbox
						...
						<span className="font-medium">{t("oacode:settings.systemNotifications.label")}</span>
					</VSCodeCheckbox>
					<div className="text-vscode-descriptionForeground text-sm mt-1">
						{t("oacode:settings.systemNotifications.description")}
					</div>
				</div>
				{showTestSystemNotification && (
					<div className="flex flex-col gap-3 pl-3 border-l-2 border-vscode-button-background">
						<Button
							...
							{t("oacode:settings.systemNotifications.testButton")}
						</Button>
					</div>
				)}
				{/* oacode_change end */}
