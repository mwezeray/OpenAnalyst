...
import { MermaidSyntaxFixer } from "@src/services/mermaidSyntaxFixer" // oacode_change
import CodeBlock from "./CodeBlock"
import { MermaidButton } from "@/components/common/MermaidButton"
import { MermaidFixButton } from "@/components/common/MermaidFixButton" // oacode_change
...
// oacode_change next line rename to originalCode to keep the difference with Roo smaller
export default function MermaidBlock({ code: originalCode }: MermaidBlockProps) {
	...
	useEffect(() => {
		...
		setIsFixing(false)
		// oacode_change end
	}, [originalCode]) // oacode_change originalCode instead of code
	...
	useDebounceEffect(
		...
		500, // Delay 500ms
		[code, isFixing, originalCode, t], // Dependencies for scheduling // oacode_change added isFixing, originalCode and t
	)
	...
	return (
		<MermaidBlockContainer>
			{isLoading && (
				<LoadingMessage>
					{isFixing /* oacode_change */ ? t("common:mermaid.fixing_syntax") : t("common:mermaid.loading")}
				</LoadingMessage>
			)}
			...
			{error ? (
				...
				<MermaidButton containerRef={containerRef} code={code} isLoading={isLoading} svgToPng={svgToPng}>
					{/* oacode_change start switched from ref to dangerouslySetInnerHTML */}
					<SvgContainer
						onClick={handleClick}
						ref={containerRef}
						$isLoading={isLoading}
