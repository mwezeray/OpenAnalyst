...
export const MarketplaceItemCard: React.FC<MarketplaceItemCardProps> = ({ item, filters, setFilters, installed }) => {
	...
	const [installModalVersion, setInstallModalVersion] = useState(0) // oacode_change
	...
	const handleInstallClick = () => {
		...
		setInstallModalVersion((prev) => prev + 1) // oacode_change
	...
	return (
		<>
			...
			<MarketplaceInstallModal
				key={`install-modal-${item.id}-${installModalVersion}` /* oacode_change */}
				item={item}
