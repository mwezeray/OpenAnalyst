...
import { useTaskSearch } from "./useTaskSearch" // oacode_change
...
export const BatchDeleteTaskDialog = ({ taskIds, ...props }: BatchDeleteTaskDialogProps) => {
	...
	const { tasks } = useTaskSearch() // oacode_change
	const { onOpenChange } = props

	const favoritedTasks = tasks.filter((task) => taskIds.includes(task.id) && task.isFavorited) // oacode_change
	const hasFavoritedTasks = favoritedTasks.length > 0 // oacode_change
