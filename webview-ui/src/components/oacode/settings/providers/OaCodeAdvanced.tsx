...
type OaCodeAdvancedProps = {
	apiConfiguration: ProviderSettings
	setApiConfigurationField: (field: keyof ProviderSettings, value: ProviderSettings[keyof ProviderSettings]) => void
	routerModels?: RouterModels
}

export const OaCodeAdvanced = ({ apiConfiguration, setApiConfigurationField, routerModels }: OaCodeAdvancedProps) => {
	const { t } = useAppTranslation()
	const { data: openRouterModelProviders } = useOpenRouterModelProviders(
		apiConfiguration?.oacodeModel,
		undefined,
		undefined,
		{
			enabled:
				...
				apiConfiguration.oacodeModel in routerModels.openrouter,
		},
	)
				...
				<div>
					<div className="flex items-center gap-1">
						<label className="block font-medium mb-1">
							{t("oacode:settings.provider.providerRouting.title")}
						</label>
						<a href={`https://openrouter.ai/${apiConfiguration?.oacodeModel}/providers`}>
					...
					<div className="text-sm text-vscode-descriptionForeground mt-1">
						{t("oacode:settings.provider.providerRouting.description")}
					</div>
				</div>
			)}
		</>
